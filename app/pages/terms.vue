...
const sections = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Paid plans determine placement inside LaunchLog. Within any single browse or category page, Featured listings appear before Standard listings. Each directory page shows at most three Featured listings; Featured listings beyond that carry onto the following pages, so a Standard listing that completes a row on one page can appear before a Featured listing carried onto the next page. Featured listings also appear in the homepage Featured section. Within each plan, the order is re-seeded once per calendar day (UTC); it is not a fixed rotation and we do not guarantee any exposure cadence. The homepage Featured section displays up to three Featured listings at a time, so a Featured listing is eligible for one of those slots rather than guaranteed one.' },
