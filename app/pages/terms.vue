...
const sections = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Paid plans determine placement inside LaunchLog. Within any single browse or category page, Featured listings appear above Premium listings, and Premium above Basic. Directory pages are composed from a fixed number of visual slots rather than a fixed number of records, so a Basic listing that completes a row on one page can appear before a paid listing carried onto the next page; Featured listings also appear in the homepage Featured section. Within each plan, the order is re-seeded once per calendar day (UTC); it is not a fixed rotation and we do not guarantee any exposure cadence. The homepage Featured section displays up to three Featured listings at a time, so a Featured listing is eligible for one of those slots rather than guaranteed one.' },
